...
extern int mt6370_set_for_ffc(bool suspend);
...
extern int oplus_voocphy_get_fast_chg_type(void);
...
static int oplus_mt6370_hardware_init(void)
{
	...
	if (gpio_is_valid(g_oplus_chip->normalchg_gpio.chargerid_switch_gpio) && (oplus_chg_get_wait_for_ffc_flag() == false)) {
		mt6370_set_for_ffc(false);
	}
...
int oplus_mt6370_charging_current_write_fast(int chg_curr)
{
	...
	if (pinfo->data.dual_charger_support) {
		...
		if (g_oplus_chip->dual_charger_support && g_oplus_chip->slave_charger_enable && (pdqc_flag == true)) {
...
static bool is_support_chargerid_check(void)
{
	...
	if (!chip) {
		chg_err("oplus_chg_chip not ready!\n");
		return false;
	}
...
void oplus_check_fastchg_to_dummy(void)
{
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	if (oplus_voocphy_get_fastchg_dummy_start() && (oplus_voocphy_get_fast_chg_type() != 0)) {
		chg_debug("turn on fastchg to dummy!\n");
